...
- README test badge 981 → 1016 across all ten languages. `.gitignore`: duplicate `.claude/` line removed, trailing newline restored.
...
- An Oracle SID / alias form (`jdbc:oracle:thin:u/p@HOST:1521:SID`) whose password itself contains `@` is masked only up to that `@`; the EZConnect and TNS forms have an unambiguous anchor and are masked whole.
...
- In a tree that mixes `controller/{domain}/` (A) with `{domain}/controller/` (B), Pattern A claims it and the B-shaped domains are dropped unless they also have a `service/` directory the supplementary scan can find. Pre-existing since v2.4.0 and unchanged here, but worth stating: it surfaced during this release's review because the `Object.prototype` crash above had been *masking* it — Pattern A silently failed to register on such a tree, which let Pattern B run instead. With the crash fixed, those trees follow the normal A-wins rule.
