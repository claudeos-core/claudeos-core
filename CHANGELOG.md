...
- [`2.5.2`](#252--2026-09-09) — `web/` recognized as the HTTP layer (eGovFrame / pre-Boot SI layouts), `*/impl/` sub-layer counted, directory fallback no longer invents domains, deep-sweep no longer double-counts controllers, URL passwords holding a raw `/` `?` `#` redacted whole, legacy tree walks prune IDE/build output
...
Patch release. v2.5.1 let pre-Boot Java projects past `init`'s "No language detected"; this one fixes what the scanner then did with them. Four of the five scanner defects produced *plausible wrong numbers* rather than a visible failure — `controllers: 0` for a project full of controllers — and those numbers are what Pass 3 reads. Also closes the one credential shape that no rule backstopped. No new files, no new dependencies; `detectStack()` is decomposed along the way (pure move, see **Changed**). `project-analysis.json` gains one optional field (`stack.envInfo.portVars`) and one optional array (`stack.envInfo.credentialWarnings`); no existing field changes type or meaning. Test suite: 974 → **997 / 997** pass (23 added, 1 assertion superseded on purpose — see **Behavior changes**).

### Fixed — safety

- **A URL password containing a raw `/`, `?` or `#` is no longer written verbatim.** v2.5.0 masks userinfo between `://` and the last `@` of the authority, and deliberately does not rewrite an `@` that appears after the first `/` — such an `@` normally belongs to the path (`https://cdn.example.com/npm/@scope/pkg`) and rewriting it would replace the real host. `postgres://app:pa/ss@db:5432/app` therefore passed through unchanged, and because its key is `DATABASE_URL` the key-name rule did not backstop it either: it was the one remaining combination that could put a plaintext password into `project-analysis.json`, which Pass 3/4 prompts instruct the model to read. v2.5.1 documented this as an accepted limit on the grounding that such a value is not a valid URL. That grounding was too generous — the base64 alphabet contains `/`, so auto-generated passwords hit this shape routinely. A new backstop (`hasUnmaskedUrlCredentials`) recognizes it precisely: the text between `://` and the first `/`, `?` or `#` must look like `user:password` (it holds a `:`, and the part after that `:` is **not** all digits, so a `host:port` is excluded) **and** an `@` must appear past that delimiter, meaning the real authority ends beyond the point the masking regex was forced to stop. When both hold, the **whole value** is dropped (`***REDACTED***`) rather than partially rewritten — once the authority is ambiguous the host cannot be located reliably, and a lost host is a far cheaper failure than a leaked credential. `init` names the affected keys in its Phase 1 summary (key names only, never any part of the value) so a silently missing `DATABASE_URL` is not mistaken for a detection bug. Both corpora are pinned as tests: five flagged shapes (raw `/`, raw `?`, several `/`, raw `#`, a base64 password) and ten that must **not** flag (`postgres://u:p@host/db`, `app:p@ss@db/app`, scoped npm paths, `@` in path or query, `host:8080/img/@2x.png`, `https://user@host/path`, `mailto:`, and the `?password=` form `PARAM_RE` already handles).
- **`extractPort()` no longer runs against redacted values.** `stack-detector` split the env vars into backend and frontend port keys by reading `envInfo.vars` — the *redacted* map. No current key-name rule intersects a port, host or api-target key (asserted across nine port-key variants since v2.5.1), so nothing was broken; but port detection depended structurally on the redaction rule set, and a future blanket rule would have killed it silently. `readStackEnvInfo` now also returns `envInfo.portVars`: the unredacted value of every key on the port whitelist, **digits only**. That constraint is what keeps it from becoming a back door — a port is digits by definition, so if a whitelisted key ever also matches a secret-name rule its secret value is simply left out. Flagged as a structural follow-up in v2.5.1's known limits; now closed.

### Fixed — detection accuracy

- **`web/` is recognized as the HTTP layer.** eGovFrame (전자정부 표준프레임워크) and the pre-Boot enterprise projects that copied its layout name the controller layer `web/`, not `controller/`. Every pattern in `scan-java` was written against the literal segment `controller`, so an eGovFrame-shaped project reported **`controllers: 0` for every domain** — the domain names were right, the counts were silently wrong, and both went straight into `pass3-context.json`. The promotion is gated on the project holding **no `controller/` directory anywhere**: every project that reached Pattern A/B/C/D through a `controller/` dir is unchanged to the byte (v2.5.1 and v2.5.2 were run side by side on the same synthetic fixture trees). A project with no `controller/` dir at all previously fell through to the directory fallback and reported its base package as a domain (`demo`, `acme`); it now reports real domains — see **Behavior changes**. Two further guards keep the rule from firing on a `web/` that is not an HTTP layer — at least one `web/*Controller.java` must exist (a `config/web/WebMvcConfig.java` must not manufacture a `config` domain), and `{domain}/adapter/in/web/` is excluded, since that is Pattern E's own spelling and reading its `web` as a layer would yield a domain named `in`. The segment is threaded through the root-package prefix count, the flat-layout class-stem test, Pattern B/D, Pattern C and the deep-sweep classifier. `user/web/UserController.java` + `user/service/…` now yields `user` with `controllers: 1`; before, `controllers: 0`.
- **`*/impl/` sub-layers are counted.** Interface-plus-implementation is eGovFrame's default and common throughout enterprise Java, but the layer globs use `*`, which does not cross `/`: `user/service/UserService.java` was counted and `user/service/impl/UserServiceImpl.java` was dropped. Added as separate, non-overlapping globs for Patterns A (`service/{domain}/impl`, `service/impl/{domain}`), B/D (`{domain}/service/impl`, `{domain}/{mapper,repository,dao}/impl`) and C (`service/impl/{Domain}*`), so no file can be counted twice. eGovFrame also parks its DAO in `service/impl/`; counting those as services — one bucket, once — is deliberate. The impl globs are kept OUT of `standardCount`, which is not a file count but the trigger for the Pattern B/D deep-sweep — it asks whether the canonical layer globs described the domain at all. Counting impl into it made a domain whose only service file was `service/impl/UserServiceImpl.java` look canonical, suppressing the sweep and with it the catch-all that classifies non-canonical layers (`gateway/`, `listener/`, …) as services; that domain then reported 2 of its 4 files. This changes domain file counts for **existing** projects that use `service/impl/`, including Spring Boot ones (see **Behavior changes**).
- **The directory fallback no longer invents domains.** When no primary pattern claims a tree, `scan-java` walks directory names looking for layer segments. It credited **both** the segment before the layer dir and the segment after it, so one file created two domains: `example/service/impl/UserServiceImpl.java` produced a domain `example` (the base package) *and* a domain `impl` (an implementation folder), and a layer-first tree `com/acme/controller/user/X.java` produced `acme` beside the real `user`. A path is either `{domain}/{layer}/` or `{layer}/{domain}/` — never both. Three corrections: one domain per file, preferring the segment **before** the layer whenever it qualifies; a `NON_DOMAIN` list so `impl`, `factory`, `handler`, `strategy` and similar role folders are never domains; and the last segment of `rootPackage` is never a domain, the same reasoning `isFlatLayerPath` already applies to the primary patterns. The precedence matters more than it looks: `{domain}/{layer}/{subpackage}/File.java` and `{layer}/{domain}/File.java` are structurally identical from the walk's point of view, so preferring the segment after the layer would read `order/service/query/OrderQueryHandler.java` as a domain named `query` and lose `order` entirely — worse than the old double-counting, which at least kept both. `{layer}/{domain}/` still resolves, because the segment before the layer there is the base package, which `rootPackage` disqualifies; that layout is also normally claimed by Pattern A long before this fallback runs.
- **Tightening that fallback cannot produce zero domains.** The three corrections above are right individually and, on a tree whose only signal was the junk they remove, would together have yielded **no** domains — which aborts `init` outright (`domain-groups.json has invalid totalGroups: 0`). Trading wrong output for no output is not an improvement. A class-name last resort now runs **only** when the directory walk found nothing, reading the domain off the layer-suffixed class name exactly as Pattern C does for flat layouts (longest suffix first, so `UserServiceImpl` yields `user`, not `userservice`). A legacy tree holding only `service/impl/UserServiceImpl.java` + `dao/UserDao.java` yields `user` — grounded in a real identifier rather than in a directory that happens to sit nearby. Because it is gated on an empty result it can never alter a project that already had domains.

- **Class-name-derived domains strip role prefixes.** `Abstract`, `Default`, `Simple`, `Generic` and — most importantly — eGovFrame's `Egov` describe a class's role, not the capability it serves. Without stripping, every class-name-derived domain in an eGovFrame tree is `egovuser`, `egovsample`, and `AbstractServiceImpl` becomes a domain called `abstract`. The stripping applies everywhere a domain is read off a class name: Pattern C (which is exactly where the `web/` rule routes the single-domain eGovFrame sample template, since its one domain is absorbed into the root package by the 80% rule), the flat-controller re-attach, and the class-name last resort of the directory fallback. On the Pattern C path the layer globs match **both** the raw and the stripped stem (`{EgovSample,Sample}*`), because eGovFrame names its service `EgovSampleServiceImpl` but its DAO `SampleDAO`, and a glob built from either stem alone drops the other file. Prefixes are stripped repeatedly and the result is re-checked against the skip lists, so `AbstractBaseServiceImpl` reduces to `base` and is dropped; a controller whose stem strips to nothing (`AbstractController`) keeps its literal name rather than vanishing. The prefix must be followed by an uppercase letter, so `EgovernanceService` keeps its name.
- **The deep-sweep no longer double-counts.** When a Pattern B/D domain has zero service/mapper/dto/xml files, `scan-java` re-walks the whole domain tree to catch non-canonical layouts. That sweep re-counted the controllers the pattern loop had already counted, so a domain whose only file was `user/controller/UserController.java` reported `controllers: 2` and `totalFiles: 2`. Latent since v2.4.0 and effectively invisible, because it needs a domain with a controller and nothing else — but the `web/` support above routes eGovFrame domains straight into exactly that shape, so the release's headline fix would have shipped inflated counts. The four counters are reset before the sweep, which re-classifies the domain tree from scratch and is the single source of truth in that branch. Affects existing Spring Boot projects too, wherever a Pattern B/D domain holds only controllers.

### Changed

- **The legacy JVM tree walks prune IDE and build output.** `jarIgnore` — shared by the `**/{WEB-INF/lib,lib,libs}/**/*.jar`, `**/WEB-INF/web.xml` and Spring-XML globs — gains `out/` (IntelliJ), `bin/` (Eclipse's default output dir), `.gradle/`, `.idea/` and `.svn/`. None of them holds evidence those blocks read, but all three globs are rooted at `**` and run on **every** framework-less Java project, not only legacy ones, so they were walked every time. This is the only cheap lever on walk cost; the globs themselves are unchanged.

- **`detectStack()` is decomposed.** It was one 1,371-line function, and the v2.5.1 legacy-JVM pass sat inside it — which is precisely what made the four scanner defects above expensive to locate. Two extractions, both pure moves verified line-by-line against the v2.5.1 source: `absorbMavenPom` (29 lines) becomes a module-level function taking `stack` as its first parameter instead of closing over it, and the legacy-JVM evidence pass (223 lines) becomes `detectLegacyJvm(ROOT, stack, languageFromPackageJson)`. The pass was already self-contained — `jvmMayClaim`, `parked`, `jarIgnore`, `referencedJars` and the `anyJavaSources` memo are all local to it, and the provisional-language reclaim it opens is also settled inside it — so `absorbMavenPom` was its only tie to the enclosing scope. `stack` is still mutated in place rather than returned, so the call site reads exactly like the inline block it replaces. No reordering, no renaming, no behavior change: `detectStack()` 1,371 → **928** lines.

### Behavior changes

Five observable changes, all deliberate:

- `maskUrlCredentials("postgres://app:pa/ss@db:5432/app")` returns `"***REDACTED***"` instead of the value verbatim, and so does the same value with a query string (`…/app?sslmode=require&password=x`) — the backstop is gated on whether the userinfo rule matched, not on whether anything else in the value changed. The v2.5.0 assertion pinning the old result is superseded, not relaxed — see **Fixed — safety**. Percent-encode the password (`/` as `%2F`) to keep scheme, host and path visible in the generated docs; `init` tells you which key it was.
- A Java project with **no `controller/` directory anywhere** — the Spring-guides layout that names the HTTP package `web/`, a `rest/`-only API, or a service/repository-only module — previously reported its base package as a single junk domain (`demo`, `acme`) and now reports its real domains from class names (`UserController` → `user`, Pattern C). Projects that do hold a `controller/` dir are unchanged.
- A Pattern C controller whose class name starts with `Abstract`, `Default`, `Simple`, `Generic` or `Egov` followed by an uppercase letter names its domain without that prefix (`DefaultUserController` → `user`, was `defaultuser`). The layer globs still match the prefixed siblings, so no file is lost.
- Domains that use a `*/impl/` sub-layer report higher `services` / `mappers` counts, and therefore a higher `totalFiles`. This feeds `domain-grouper`'s 40-files-per-group split, so such a project may be batched into a different number of Pass 3 groups than it was under v2.5.1. The counts were under-reported before; the new ones are the real file counts.
- A Pattern B/D domain that holds only controllers reports half its previous `controllers` and `totalFiles`, because those counts were doubled (see **Fixed — detection accuracy**). Like the `*/impl/` change, this can move such a project across a `domain-grouper` group boundary.

### Known limits

- The `web/`-as-HTTP-layer rule is all-or-nothing per project: a tree that uses `controller/` in one module and `web/` in another keeps the v2.5.1 behavior for the `web/` module (`controllers: 0` there). Making it per-module needs a module-scoped flag and is a separate change. The gate is deliberate — it is what guarantees every project that has a `controller/` dir is unchanged.
- The whole-value drop has one known false positive: a **bracketed IPv6 host** with an `@` later in the path (`http://[::1]:8080/img/@2x.png`) is dropped, because `[::1]:8080` reads as `user:password` to the backstop. No credential is leaked; the value is lost from `project-analysis.json` and `init` names the key. Recorded here so it is not mistaken for a detection bug.
...
None required. The new `envInfo` fields are additive. If a project's `.env` holds a URL password containing a raw `/`, `?` or `#`, that value is now dropped from `project-analysis.json` and `init` says so in Phase 1 — percent-encode it and re-run to restore the host. Java projects using a `web/` HTTP layer or a `*/impl/` sub-layer will report different (correct) domain counts on the next `init`, and so will any project — Spring Boot included — with a Pattern B/D domain that holds only controllers, whose count was previously doubled. Review the Phase 2 domain summary the first time; if a project sits near a `domain-grouper` group boundary its Pass 3 batching may shift.
