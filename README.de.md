...
**Ihre Layouts werden gescannt, nicht nur erkannt (v2.5.2).** eGovFrame und die Pre-Boot-Projekte, die es kopiert haben, nennen die HTTP-Schicht `web/` statt `controller/` und teilen Services in `service/` + `service/impl/` auf — bis v2.5.2 wurde ein solches Projekt daher korrekt erkannt und meldete anschließend für jede Domain `controllers: 0`. Beide Formen werden jetzt gelesen: `web/` gilt als Controller-Schicht für jedes Projekt, das nirgends ein `controller/`-Verzeichnis enthält (mindestens ein `web/*Controller.java` ist erforderlich, damit eine `config/web/WebConfig.java` niemals eine Domain erzeugt; `adapter/in/web/` aus Pattern E ist ausgenommen), und `*/impl/`-Dateien werden zusammen mit den Interfaces gezählt, die sie implementieren. Passt überhaupt kein Layout-Muster, stammen die Domains aus den schichtbenannten Klassennamen (`UserServiceImpl` → `user`) statt aus einem zufällig danebenliegenden Verzeichnis — ein Legacy-Baum liefert damit seine echten Domains und nicht eine einzige, die nach seinem Basispaket benannt ist.
