...
**1. Step A — Scanner (deterministisch, ohne LLM).** Ein Node.js-Scanner durchläuft den Projekt-Root, liest `package.json`, `build.gradle`, `build.gradle.kts`, `pom.xml` und `pyproject.toml`, parst `.env*`-Dateien: sensible Schlüssel wie `PASSWORD/SECRET/TOKEN/JWT_SECRET/...` werden zu `***REDACTED***`, Zugangsdaten **innerhalb** von Connection-Strings zu `***:***` maskiert, während Schema, Host, Port und Pfad lesbar bleiben — für URL-Userinfo, `?password=`-Parameter, Go-/MySQL-DSNs und Oracle-JDBC-DSNs gleichermaßen; ein Wert, dessen Zugangsdaten-Grenze mehrdeutig ist, wird ganz verworfen und sein Schlüssel gemeldet, statt halb maskiert ausgegeben zu werden. Anschließend klassifiziert er das Architekturmuster (6 Java-Patterns A–F (inklusive des Package-by-Feature-Layouts ohne Layer-Verzeichnisse), Kotlin CQRS oder Multi-Module, Next.js App- gegenüber Pages-Router, FSD, Components-Pattern), erkennt Domänen und baut eine explizite Allowlist aller tatsächlich existierenden Quellpfade. Das Ergebnis landet in `project-analysis.json`, der Single Source of Truth für alles, was danach kommt.
...
**Package-by-Feature-Java wird erkannt (v2.5.3).** Das Layout, das Springs eigener Leitfaden *Structuring Your Code* empfiehlt — `com/acme/order/{OrderController,OrderService,OrderRepository}.java`, ohne Layer-Verzeichnisse — ergab bisher **null Backend-Domains**; in einem gemischten Baum überlebte nur die eine Domain mit `controller/`-Verzeichnis, die layer-losen verschwanden stillschweigend. Pattern F registriert jetzt jedes Feature-Package als Domain und ordnet dessen Dateien über das Klassennamen-Suffix zu. Ein Baum, der Layer-first (`controller/{domain}/`) mit Domain-first (`{domain}/controller/`) mischt, behält **beide** Domain-Arten, und keine Domain wird mehr mit Dateizahl 0 ausgegeben.
