...
**Sus layouts se escanean, no solo se detectan (v2.5.2).** eGovFrame y los proyectos pre-Boot que lo copiaron llaman `web/` a la capa HTTP en lugar de `controller/`, y separan los servicios en `service/` + `service/impl/`; hasta v2.5.2, un proyecto así se detectaba correctamente y luego reportaba `controllers: 0` en todos sus dominios. Ahora se leen ambas formas: `web/` cuenta como capa de controladores en cualquier proyecto que no contenga ningún directorio `controller/` (se exige al menos un `web/*Controller.java`, de modo que un `config/web/WebConfig.java` nunca fabrica un dominio, y se excluye el `adapter/in/web/` del Pattern E), y los archivos `*/impl/` se cuentan junto a las interfaces que implementan. Cuando ningún patrón de layout encaja, los dominios se obtienen de los nombres de clase con sufijo de capa (`UserServiceImpl` → `user`) y no del directorio que casualmente esté al lado: un árbol heredado devuelve sus dominios reales en vez de uno solo con el nombre de su paquete base.
