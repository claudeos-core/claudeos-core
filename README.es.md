...
**1. Etapa A — Scanner (reproducible, sin LLM).** Un scanner en Node.js recorre la raíz del proyecto, lee `package.json`, `build.gradle`, `build.gradle.kts`, `pom.xml` o `pyproject.toml`, parsea los archivos `.env*` (las claves sensibles como `PASSWORD/SECRET/TOKEN/JWT_SECRET/...` pasan a `***REDACTED***`; las credenciales que viven **dentro** de una cadena de conexión se enmascaran a `***:***` dejando legibles el esquema, el host, el puerto y la ruta — vale para el userinfo de una URL, los parámetros `?password=`, los DSN de Go/MySQL y los DSN de Oracle JDBC; y un valor cuyo límite de credencial es ambiguo se descarta entero y se informa su clave, nunca se publica a medio enmascarar), clasifica el patrón de arquitectura (los 6 patrones A–F de Java (incluido el diseño package-by-feature sin directorios de capa), CQRS o multi-módulo en Kotlin, App Router o Pages Router en Next.js, FSD, components-pattern), descubre los dominios y compone una lista explícita con todas las rutas de archivos fuente que existen. El resultado se vuelca en `project-analysis.json`, la única fuente de verdad para lo que viene después.
...
**Java package-by-feature también se detecta (v2.5.3).** El diseño que recomienda la propia guía *Structuring Your Code* de Spring — `com/acme/order/{OrderController,OrderService,OrderRepository}.java`, sin directorios de capa — producía **cero dominios de backend**; en un árbol mixto sobrevivía únicamente el dominio que sí tenía un directorio `controller/` y los demás desaparecían en silencio. Ahora el Pattern F registra cada paquete de feature como dominio y clasifica sus archivos por el sufijo del nombre de clase. Un árbol que mezcla capa-primero (`controller/{domain}/`) con dominio-primero (`{domain}/controller/`) conserva **ambos** tipos de dominio, y ya no se emite ningún dominio con cero archivos.
