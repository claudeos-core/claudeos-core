...
**1. Étape A — Scanner (reproductible, sans LLM).** Un scanner Node.js parcourt la racine du projet, lit `package.json`, `build.gradle`, `build.gradle.kts`, `pom.xml` ou `pyproject.toml`, et analyse les fichiers `.env*`. Les clés sensibles (`PASSWORD/SECRET/TOKEN/JWT_SECRET/...`) deviennent `***REDACTED***`, et les identifiants situés **à l'intérieur** d'une chaîne de connexion sont masqués en `***:***` tout en laissant lisibles le schéma, l'hôte, le port et le chemin — cela vaut pour l'userinfo d'une URL, les paramètres `?password=`, les DSN Go/MySQL et les DSN Oracle JDBC ; une valeur dont la frontière d'identifiant est ambiguë est abandonnée en entier et sa clé signalée, jamais publiée à demi masquée. Le scanner classe ensuite le pattern d'architecture (6 patterns Java A–F (dont la disposition package-by-feature sans répertoires de couche), Kotlin CQRS ou multi-module, Next.js App ou Pages Router, FSD, components-pattern), identifie les domaines et construit une liste blanche explicite qui recense chaque chemin de fichier source réellement présent. Le tout finit dans `project-analysis.json`, source unique de vérité pour la suite.
...
**Le Java package-by-feature est détecté (v2.5.3).** La disposition que recommande le guide *Structuring Your Code* de Spring lui-même — `com/acme/order/{OrderController,OrderService,OrderRepository}.java`, sans répertoires de couche — donnait jusqu'ici **zéro domaine backend** ; dans un arbre mixte, seul le domaine doté d'un répertoire `controller/` survivait et les autres disparaissaient silencieusement. Le Pattern F enregistre désormais chaque package de feature comme un domaine et classe ses fichiers d'après le suffixe du nom de classe. Un arbre mêlant couche-d'abord (`controller/{domain}/`) et domaine-d'abord (`{domain}/controller/`) conserve **les deux** types de domaine, et plus aucun domaine n'est émis avec zéro fichier.
