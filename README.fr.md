...
**Leurs arborescences sont scannées, pas seulement détectées (v2.5.2).** eGovFrame et les projets pré-Boot qui l'ont copié nomment la couche HTTP `web/` plutôt que `controller/`, et séparent les services en `service/` + `service/impl/` — jusqu'à la v2.5.2, un tel projet était donc correctement détecté puis rapportait `controllers: 0` pour chaque domaine. Les deux formes sont désormais lues : `web/` compte comme couche de contrôleurs pour tout projet ne contenant aucun répertoire `controller/` (au moins un `web/*Controller.java` est requis, si bien qu'un `config/web/WebConfig.java` ne fabrique jamais de domaine, et le `adapter/in/web/` du Pattern E est exclu), et les fichiers `*/impl/` sont comptés avec les interfaces qu'ils implémentent. Lorsqu'aucun motif d'arborescence ne correspond, les domaines proviennent des noms de classe suffixés par leur couche (`UserServiceImpl` → `user`) et non du répertoire qui se trouve à côté par hasard : une arborescence héritée rend ses vrais domaines au lieu d'un seul nommé d'après son package de base.
