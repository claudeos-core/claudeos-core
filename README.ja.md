...
Controller のないサービスのみのドメインも `service/`、`dao/`、`aggregator/`、`mapper/`、`repository/` ディレクトリを通じて検出されます。スキップ：`common`、`config`、`util`、`core`、`front`、`admin`、`v1`、`v2` など。
...
- **テストカバレッジ** — テストスイート拡大中（現在 87 テスト、スタック検出、ドメイングルーピング、プラン検証、構造スキャン、検証ツールをカバー）
