...
Домены только с сервисами (без контроллеров) тоже обнаруживаются через директории `service/`, `dao/`, `aggregator/`, `mapper/`, `repository/`. Пропускаются: `common`, `config`, `util`, `core`, `front`, `admin`, `v1`, `v2` и т.д.
...
- **Покрытие тестами** — Расширение набора тестов (сейчас 87 тестов, покрывающих обнаружение стека, группировку доменов, валидацию планов, сканирование структуры и инструменты проверки)
