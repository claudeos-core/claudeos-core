...
[![tests](https://img.shields.io/badge/tests-1031%20passing-brightgreen?logo=node.js&logoColor=white)](https://github.com/claudeos-core/claudeos-core/actions/workflows/test.yml)
...
**1. Step A — Scanner (детерминированно, без LLM).** Сканер на Node.js обходит корень проекта, читает `package.json`, `build.gradle`, `build.gradle.kts`, `pom.xml`, `pyproject.toml`, разбирает файлы `.env*` (чувствительные ключи вроде `PASSWORD/SECRET/TOKEN/JWT_SECRET/...` превращаются в `***REDACTED***`; учётные данные **внутри** строки подключения маскируются до `***:***`, а схема, хост, порт и путь остаются читаемыми — это касается userinfo в URL, параметров `?password=`, DSN Go/MySQL и DSN Oracle JDBC; значение, у которого границу учётных данных определить нельзя, отбрасывается целиком с указанием имени ключа и никогда не публикуется наполовину замаскированным), классифицирует архитектурный паттерн (6 паттернов Java A–F (включая раскладку package-by-feature без каталогов слоёв), Kotlin CQRS / multi-module, Next.js App vs Pages Router, FSD, components-pattern), находит домены и собирает явный allowlist путей всех существующих исходных файлов. На выходе — `project-analysis.json`, единый источник истины для всех последующих шагов.
...
- `sync-checker` — соответствие диска и регистраций в `sync-map.json` по 7 отслеживаемым каталогам. **Не работает с v2.1.0:** сборка master plan удалена, поэтому `sync-map.json` пишется с пустым списком сопоставлений, и этот проверяющий возвращает `pass`, ничего не проверив. Он оставлен ради обратной совместимости; читайте его вердикт как «неприменимо», а не как доказательство. Единственное исключение — проект, обновлённый с версии до v2.1.0, в котором ещё сохранился каталог `claudeos-core/plan/`: `init` его не трогает, поэтому карта заполняется из него и проверка действительно выполняется.
...
**Java в раскладке package-by-feature тоже определяется (v2.5.3).** Раскладка, которую рекомендует собственное руководство Spring *Structuring Your Code* — `com/acme/order/{OrderController,OrderService,OrderRepository}.java`, без каталогов слоёв — раньше давала **ноль бэкенд-доменов**; в смешанном дереве выживал только домен с каталогом `controller/`, а остальные молча исчезали. Теперь Pattern F регистрирует каждый feature-пакет как домен и раскладывает его файлы по суффиксу имени класса. Дерево, где слой-первым (`controller/{domain}/`) соседствует с доменом-первым (`{domain}/controller/`), сохраняет **оба** вида доменов, и больше не выдаётся домен с нулём файлов.
...
| `sync-checker` | Соответствие файлов на диске записям в `sync-map.json` (поиск orphaned/unregistered) — **не работает, если только в проекте не остался `claudeos-core/plan/` времён до v2.1.0** | `health` (fail-on-error) |
