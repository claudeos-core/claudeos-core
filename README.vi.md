...
**1. Step A — Scanner (nhất quán, không gọi LLM).** Một Node.js scanner đi qua thư mục gốc, đọc `package.json`, `build.gradle`, `build.gradle.kts`, `pom.xml`, `pyproject.toml`, parse các file `.env*` (các key nhạy cảm như `PASSWORD/SECRET/TOKEN/JWT_SECRET/...` trở thành `***REDACTED***`; credential nằm **bên trong** connection string được che thành `***:***` trong khi scheme, host, port và path vẫn đọc được — áp dụng cho URL userinfo, tham số `?password=`, DSN Go/MySQL và DSN Oracle JDBC; giá trị có ranh giới credential không xác định được thì bị bỏ nguyên và báo tên key, không bao giờ xuất ra khi mới che một nửa), phân loại pattern kiến trúc (6 pattern A–F của Java (gồm cả layout package-by-feature không có thư mục layer); Kotlin CQRS hoặc multi-module; Next.js App Router so với Pages Router; FSD; components-pattern), tìm ra các domain, rồi dựng allowlist tường minh chứa mọi đường dẫn source thật. Output là `project-analysis.json`, đóng vai trò single source of truth cho mọi bước về sau.
...
**Java theo package-by-feature cũng được phát hiện (v2.5.3).** Layout mà chính hướng dẫn *Structuring Your Code* của Spring khuyến nghị — `com/acme/order/{OrderController,OrderService,OrderRepository}.java`, không có thư mục layer — trước đây cho ra **không domain backend nào**; trong một cây trộn layout, chỉ domain có thư mục `controller/` sống sót còn những domain không có layer thì âm thầm biến mất. Giờ Pattern F đăng ký từng package feature thành một domain và phân loại file theo hậu tố tên class. Cây trộn layer-first (`controller/{domain}/`) với domain-first (`{domain}/controller/`) giữ lại **cả hai** loại domain, và không còn domain nào có số file bằng 0.
