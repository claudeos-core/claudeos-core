...
- The claudeos-core-managed categories under `.claude/rules/` — every entry with a `NN.` prefix (`00.core/`, `10.backend/`, … `90.optional/`) — so Pass 3's "zero-rules detection" can't false-negative on stale rules

`--force` does **not** delete:
- `claudeos-core/memory/` files (your decision log and failure patterns are preserved)
- Files or folders you placed under `.claude/rules/` yourself without a `NN.` prefix (e.g. `.claude/rules/my-team-conventions.md`) — they were never generated by this tool
- Files outside `claudeos-core/` and `.claude/`

**Manual edits to generated rule files are lost under `--force`.** That's the trade-off — `--force` exists for "I want a clean slate." If you want to preserve edits, just re-run without `--force`.
...
Applies a 4-stage compaction over `failure-patterns.md` (`decision-log.md` is append-only and is never compacted):
