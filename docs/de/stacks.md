...
**Erkennung des Architekturmusters.** Der Scanner klassifiziert dein Projekt in **eines von 6 Mustern**:
...
| **F. Package-by-feature** (v2.5.3) | `order/OrderController.java`, `order/OrderService.java`, `order/OrderRepository.java` — kein Layer-Verzeichnis |

Die Muster werden in der Reihenfolge A → B/D → E → C probiert; **F läuft danach als ergänzender Durchgang über jeden Baum** und registriert jedes Feature-Paket, das direkt eine `*Controller.java` enthält und das kein früheres Muster beansprucht hat. Ein gemischter Baum behält so sowohl seine `order/controller/`-Domains als auch seine `payment/PaymentController.java`-Domains. F zählt Dateien nach Klassennamen-Suffix (`*Service`, `*Repository`/`*Mapper`/`*Dao`, `*Dto`/`*Entity`/`*Vo`, alles andere als Service), weil es kein Layer-Verzeichnis zum Auslesen gibt. Ein Controller, der direkt im Basispaket neben `*Application.java` liegt, ist die Initializr-Einzelpaket-Demo und behält die Regel von Muster C (Domain aus dem Klassennamen). (1) **Root-Package-Erkennung** wählt das längste Paket-Präfix, das ≥80 % der layer-tragenden Dateien abdeckt (deterministisch über Re-Runs). (2) **Deep-Sweep-Fallback** für Muster B/D: Liefern die Standard-Globs für eine registrierte Domäne null Dateien, globbt der Scanner erneut `**/${domain}/**/*.java` und geht jeden Datei-Pfad durch, um das nächste Layer-Verzeichnis zu finden. Das deckt Cross-Domain-Coupling-Layouts wie `core/{otherDomain}/{layer}/{domain}/` auf.
...
**Bekannte Grenzen.** Gradle-Dateien werden nicht von Kommentaren bereinigt (eine mit `//` auskommentierte Koordinate zählt weiterhin mit — bei Boot war das schon immer so). Vererbung aus einem Parent-POM außerhalb des Repositories wird nicht aufgelöst. Die Controller-Schicht `web/` von eGovFrame wird von Pattern A/B noch nicht als Schichtname erkannt. Ein Unterverzeichnis `controller/impl/` wird von Muster A als Domain namens `impl` registriert. Ein Controller, dessen Klassenname nicht auf `Controller` endet (`OrderResource`, `OrderEndpoint`), ist kein Signal für Muster F — seit v2.5.3 sagt `init` das in Phase 2, wenn ein erkanntes Backend keine Domains liefert.
