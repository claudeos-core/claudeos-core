...
**Détection du pattern d'architecture.** Le scanner classe ton projet dans **un des 6 patterns** :
...
| **F. Package-by-feature** (v2.5.3) | `order/OrderController.java`, `order/OrderService.java`, `order/OrderRepository.java` — aucun répertoire de couche |

Les patterns sont essayés dans l'ordre A → B/D → E → C ; **F s'exécute ensuite comme passe supplémentaire sur tous les arbres**, enregistrant chaque paquet de feature qui contient directement un `*Controller.java` et qu'aucun pattern précédent n'a revendiqué. Un arbre mixte conserve ainsi à la fois ses domaines `order/controller/` et ceux de `payment/PaymentController.java`. F compte les fichiers d'après le suffixe du nom de classe (`*Service`, `*Repository`/`*Mapper`/`*Dao`, `*Dto`/`*Entity`/`*Vo`, tout le reste comme service), faute de répertoire de couche à lire. Un controller placé directement dans le paquet de base à côté de `*Application.java` est la démo mono-paquet d'Initializr et garde la règle du pattern C (domaine tiré du nom de classe). (1) **détection du root-package** prend le préfixe de package le plus long qui couvre ≥80 % des fichiers porteurs de couche (déterministe entre les réexécutions) ; (2) **deep-sweep fallback** pour Pattern B/D : quand les globs standard renvoient zéro fichier pour un domaine enregistré, le scanner re-globalise `**/${domain}/**/*.java` et parcourt le path de chaque fichier pour trouver le répertoire de couche le plus proche, attrapant les layouts à couplage cross-domaine comme `core/{otherDomain}/{layer}/{domain}/`.
...
**Limites connues.** Les fichiers Gradle ne sont pas débarrassés de leurs commentaires (une coordonnée commentée par `//` compte toujours — c'est le cas depuis toujours pour Boot). L'héritage depuis un POM parent extérieur au dépôt n'est pas résolu. La couche de contrôleurs `web/` d'eGovFrame n'est pas encore reconnue comme nom de couche par les Patterns A/B. Un sous-répertoire `controller/impl/` est enregistré par le pattern A comme un domaine nommé `impl`. Un controller dont le nom de classe ne se termine pas par `Controller` (`OrderResource`, `OrderEndpoint`) n'est pas un signal du pattern F — depuis la v2.5.3, `init` le signale en phase 2 lorsqu'un backend détecté ne produit aucun domaine.
