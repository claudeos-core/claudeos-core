...
Quand Claude a fini de générer la documentation, le code vérifie la sortie via **5 validators distincts**. Aucun n'appelle de LLM : toutes les vérifications sont déterministes.

Cette page couvre ce que vérifie chaque validator, le fonctionnement des niveaux de sévérité, et la lecture de la sortie.
...
Les LLMs sont non déterministes. Même avec des prompts à injection de faits (l'[allowlist de paths source](architecture.md#les-prompts-à-injection-de-faits-coupent-les-hallucinations)), Claude peut encore :

- Sauter une section requise sous pression de contexte.
- Citer un path qui matche l'allowlist à un détail près (par ex. `src/feature/routers/featureRoutePath.ts` inventé depuis un répertoire parent + un nom de constante TypeScript).
- Produire des cross-references incohérentes entre standards et rules.
- Redéclarer le contenu de la Section 8 ailleurs dans CLAUDE.md.

Les validators attrapent ces sorties cassées en silence avant la livraison.
...
**Pourquoi ça compte en pratique :** un CLAUDE.md généré avec `--lang ja` et un autre avec `--lang en` paraissent complètement différents pour un humain, mais `claude-md-validator` produit des verdicts pass/fail byte-for-byte identiques sur les deux. Pas de dictionnaire à maintenir par langue.
...
|---|---|
| `STALE_PATH` | Toute référence `src/...` se terminant par `.ts .tsx .js .jsx .mjs .cjs .vue .svelte .java .kt .kts .py .xml .sql` dans `.claude/rules/**` ou `claudeos-core/standard/**` doit résoudre vers un fichier réel : directement sous la racine du projet, ou sous n'importe quel répertoire de module jusqu'à trois niveaux de profondeur contenant `src/` (workspaces JS `apps/*`, `packages/*` ; modules Gradle/Maven tels que `api/src/main/java/…` ; layouts imbriqués tels que `servers/query/<x>/src/…`). Les blocs de code fencés et les chemins placeholder (`src/{domain}/feature.ts`) sont exclus ; les extensions de configuration (`.yml`, `.properties`) ne sont délibérément pas vérifiées car elles sont généralement citées comme noms de profil illustratifs. |
...
L'exception orchestrator/sub-skill : une sub-skill enregistrée à `{category}/{parent-stem}/{NN}.{name}.md` est considérée couverte dès qu'un orchestrator à `{category}/*{parent-stem}*.md` est mentionné dans CLAUDE.md.

**Sévérité :** content-validator s'exécute au tier **advisory** : apparaît dans la sortie mais ne bloque pas la CI. Le raisonnement : relancer Pass 3 ne garantit pas de fixer les hallucinations LLM, donc bloquer ferait deadlock les utilisateurs dans des boucles `--force`. Le signal de détection (exit non-zero + entrée `stale-report`) suffit aux pipelines CI et au triage humain.
...
Le check pass2 est **stack-aware** : il lit `project-analysis.json` pour déterminer backend/kotlin/cqrs et ajuste les sections attendues.

**Sévérité :** s'exécute au tier **warn-only**, fait remonter les problèmes sans bloquer la CI.
...
**3 modes :**
- `--check` (défaut) : détecte uniquement le drift
...
Conservé dans la suite de validators pour les utilisateurs qui maintiennent à la main des fichiers plan pour du backup ad hoc.
...
1. **Disk → Plan :** parcourt 7 répertoires trackés (`.claude/rules`, `standard`, `skills`, `guide`, `database`, `mcp-guide`, `memory`) + `CLAUDE.md`. Reporte les fichiers existants sur disque mais non enregistrés dans `sync-map.json`.
2. **Plan → Disk :** reporte les paths enregistrés dans `sync-map.json` qui n'existent plus sur disque (orphaned).

**Exit code :** seuls les fichiers orphaned causent exit 1. Les fichiers unregistered sont informationnels (un projet v2.1.0+ a zéro path enregistré par défaut, c'est le cas commun).
...
Tous les checks échoués n'ont pas la même gravité. Le `health-checker` orchestre les validators d'exécution avec une sévérité à trois niveaux :

| Tier | Symbole | Comportement |
|---|---|---|
| **fail** | `❌` | Bloque la complétion. La CI sort non-zero. À corriger. |
...
Le système 3-tier a été ajouté pour que les findings du `content-validator` (potentiels faux-positifs sur des layouts inhabituels) ne bloquent pas les pipelines CI. Sans cela, chaque advisory bloquerait, et relancer `init` ne fixe pas fiablement les hallucinations LLM.
...
`claude-md-validator` trouve des problèmes **structurels** : compte de sections faux, table de fichier memory redéclarée, heading canonique sans le token anglais. Des signaux qui veulent dire **CLAUDE.md doit être régénéré**, pas des soft warnings à examiner. Le fix : relancer `init` (avec `--force` au besoin).

Les autres validators trouvent des problèmes de **contenu** : paths, entrées manifest, trous de schéma. Examinables et corrigeables à la main sans tout régénérer.
...
```bash
# Validation structurelle sur CLAUDE.md
npx claudeos-core lint

# Suite health à 4 validators
npx claudeos-core health
```

Pour la CI, `health` reste le check recommandé. Toujours rapide (pas d'appels LLM) et couvre tout sauf les checks structurels CLAUDE.md, que la plupart des pipelines CI n'ont pas besoin de vérifier à chaque commit.

Pour les hooks pre-commit, `lint` est assez rapide pour tourner à chaque commit.
...
L'exit code est non-zero uniquement pour les findings de tier `fail`. `warn` et `advisory` s'impriment sans faire échouer le build.

Le workflow CI officiel (dans `.github/workflows/test.yml`) tourne sur `ubuntu-latest`, `windows-latest`, `macos-latest` × Node 18 / 20.

---

## Quand un validator signale quelque chose qui vous paraît faux
...
**Si un validator se trompe à un niveau général** (pas juste sur votre projet), [ouvrez une issue](https://github.com/claudeos-core/claudeos-core/issues). Ces checks sont ajustés au fil du temps à partir de vrais reports.
