...
**Architecture pattern detection.** Scanner आपके project को **6 patterns में से एक** में classify करता है:
...
| **F. Package-by-feature** (v2.5.3) | `order/OrderController.java`, `order/OrderService.java`, `order/OrderRepository.java` — कोई layer directory नहीं |

Patterns A → B/D → E → C क्रम में try होते हैं; **उसके बाद F हर tree पर एक supplementary pass के रूप में चलता है** और हर उस feature package को register करता है जिसमें सीधे `*Controller.java` है और जिसे किसी पहले pattern ने claim नहीं किया। इससे mixed tree में `order/controller/` वाले domains और `payment/PaymentController.java` वाले domains — दोनों बचे रहते हैं। पढ़ने के लिए कोई layer directory नहीं होती, इसलिए F class name के suffix से files गिनता है (`*Service`, `*Repository`/`*Mapper`/`*Dao`, `*Dto`/`*Entity`/`*Vo`, बाकी सब service)। `*Application.java` के बगल में base package में सीधे रखा controller Initializr का single-package demo है और pattern C का नियम रखता है (domain class name से)। (1) **root-package detection** सबसे लंबा package prefix चुनता है जो ≥80% layer-bearing files cover करे (re-runs में deterministic); (2) Pattern B/D के लिए **deep-sweep fallback**. Standard globs किसी registered domain के लिए zero files लौटाएँ, तो scanner `**/${domain}/**/*.java` दोबारा glob करता है और हर file का path walk करके nearest layer dir ढूँढता है, `core/{otherDomain}/{layer}/{domain}/` जैसे cross-domain coupling layouts पकड़ लेता है।
...
**ज्ञात सीमाएँ.** Gradle files से comments नहीं हटाए जाते (`//` से comment किया coordinate भी गिना जाता है — Boot के लिए हमेशा से ऐसा ही रहा है)। Repository के बाहर के parent pom से inheritance resolve नहीं होती। eGovFrame की `web/` controller layer अभी Pattern A/B के लिए layer नाम के रूप में पहचानी नहीं जाती। `controller/impl/` sub-directory को pattern A `impl` नाम के domain के रूप में register करता है। जिस controller का class name `Controller` पर खत्म नहीं होता (`OrderResource`, `OrderEndpoint`) वह pattern F का signal नहीं है — v2.5.3 से जब detected backend से कोई domain नहीं मिलता तो `init` phase 2 में यह बता देता है।
