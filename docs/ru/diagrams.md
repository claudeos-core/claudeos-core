...
Визуальные референсы архитектуры. Все диаграммы — Mermaid; они автоматически отрисовываются на GitHub. Если вы читаете это в не-Mermaid просмотрщике, текстовые объяснения намеренно полны и сами по себе.
...
**Зелёный** = код (детерминированный). **Розовый** = Claude (LLM). Эти двое никогда не пересекаются на одной задаче.

---

## Pass 3 split mode

Pass 3 всегда разбивается на стадии — никогда не запускается одним вызовом, независимо от размера проекта. Это удерживает промпт каждой стадии в context window LLM, даже когда `pass2-merged.json` большой:
...
**Ключевая идея:** Pass 3a один раз читает большой input и производит маленький fact sheet. Стадии 3b/3c/3d читают только маленький fact sheet, никогда не перечитывают большой input. Это устраняет ошибки «Prompt is too long», мучившие более ранние не-split дизайны.

Для проектов с 16+ доменами 3b и 3c дополнительно делятся на батчи по ≤15 доменов. Каждый батч — это собственный вызов Claude со свежим context window.
...
Розовые блоки = Claude вызывается. Ромбические решения — это чистые проверки файловой системы; они происходят до любого вызова LLM.
...
Трёхуровневая severity означает, что CI не падает на warning-ах или advisory — только на жёстких провалах (уровень `fail`).

`claude-md-validator` запускается отдельно, потому что его находки — **структурные**: если CLAUDE.md malformed, правильный ответ — перезапустить `init`, а не молча предупреждать. Остальные validator запускаются как часть `health`, потому что их находки — content-уровневые (пути, записи manifest, пробелы схемы) — их можно просмотреть, не регенерируя всё.
...
**Розовый** = автоматически загружается Claude Code в каждой сессии (вы их вручную не загружаете). Всё остальное загружается по требованию или ссылается из автозагружаемых файлов.

Префиксы `00`/`10`/`20`/`30`/`40`/`70`/`80` появляются и в `rules/`, и в `standard/` — одна концептуальная область, разные роли (rules — это loaded directives, standards — справочные документы). Числовые префиксы дают стабильный порядок сортировки и позволяют Pass 3 orchestrator адресовать группы категорий (например, 60.memory пишется Pass 4, 70.domains пишется per-batch). Что реально триггерит Claude Code на автозагрузку правила — это glob `paths:` в его YAML frontmatter, а не число категории.

`50.sync` и `60.memory` — **только rules** (нет соответствующего `standard/` каталога). `90.optional` — **только standard** (специфика стека без принуждения).
...
Memory-файлы загружаются **по требованию**, а не всегда. Это держит контекст Claude худым во время обычного кодинга. Они подтягиваются только когда glob `paths:` правила совпадает с файлом, который Claude сейчас редактирует.
