...
Память растёт, но не разбухает. Четырёхстадийная компактация `failure-patterns.md` запускается командой:

```bash
npx claudeos-core memory compact
```

`decision-log.md` **никогда** не компактуется: по контракту он append-only («почему» за каждым решением — именно то, что обязано сохраниться), поэтому стадии ниже применяются только к `failure-patterns.md`.
