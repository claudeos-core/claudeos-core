...
**Определение архитектурного паттерна.** Сканер классифицирует проект как **один из 6 паттернов**:
...
| **F. Package-by-feature** (v2.5.3) | `order/OrderController.java`, `order/OrderService.java`, `order/OrderRepository.java` — без директории слоя |

Паттерны пробуются в порядке A → B/D → E → C; **затем F выполняется как дополнительный проход по любому дереву**, регистрируя каждый feature-пакет, который непосредственно содержит `*Controller.java` и который не забрал ни один предыдущий паттерн. Поэтому в смешанном дереве сохраняются и домены `order/controller/`, и домены `payment/PaymentController.java`. Читать директорию слоя здесь нечего, поэтому F считает файлы по суффиксу имени класса (`*Service`, `*Repository`/`*Mapper`/`*Dao`, `*Dto`/`*Entity`/`*Vo`, всё остальное — service). Контроллер, лежащий прямо в базовом пакете рядом с `*Application.java`, — это однопакетное демо Initializr, и для него сохраняется правило паттерна C (домен из имени класса). (1) **root-package detection** выбирает самый длинный package prefix, который покрывает ≥80% layer-bearing файлов (детерминированно между перезапусками). (2) **deep-sweep fallback** для Pattern B/D: когда стандартные globs возвращают ноль файлов для зарегистрированного домена, сканер переглобирует `**/${domain}/**/*.java` и идёт по пути каждого файла, ища ближайший layer-каталог. Так ловятся cross-domain-coupling layouts вроде `core/{otherDomain}/{layer}/{domain}/`.
...
**Известные ограничения.** Файлы Gradle не очищаются от комментариев (координата, закомментированная через `//`, по-прежнему учитывается — для Boot так было всегда). Наследование от родительского pom вне репозитория не разрешается. Слой контроллеров `web/` в eGovFrame пока не распознаётся как имя слоя для Pattern A/B. Подкаталог `controller/impl/` регистрируется паттерном A как домен с именем `impl`. Контроллер, имя класса которого не заканчивается на `Controller` (`OrderResource`, `OrderEndpoint`), не является сигналом паттерна F — начиная с v2.5.3 `init` сообщает об этом в фазе 2, когда обнаруженный бэкенд не даёт ни одного домена.
...
**Маскирование чувствительных переменных:** ключи, совпадающие с `PASSWORD`, `PASS`, `PW`, `PASSPHRASE`, `SECRET`, `TOKEN`, `API_KEY`, `CREDENTIAL`, `PRIVATE_KEY`, `JWT_SECRET`, `SSH_KEY`, `MASTER_KEY`, `SERVICE_ACCOUNT` и т.п., автоматически меняются на `***REDACTED***` перед копированием в `project-analysis.json`. У любого другого значения в форме URL (`DATABASE_URL`, `REDIS_URL`, `MONGO_URI`, `jdbc:postgresql://…`) маскируются только учётные данные — они заменяются на `***:***`, а схема, хост, порт и путь сохраняются (`postgres://***:***@db.internal:5432/app`). Тип DB по-прежнему различим, пароль в файл не попадает. Собственное определение типа DB сканером читает сырой текст `.env` напрямую и не затрагивается. Начиная с v2.5.2 значение, чью userinfo это правило не может переписать, отбрасывается ЦЕЛИКОМ (`***REDACTED***`), а не пропускается: пароль с необработанным `/`, `?`, `#` или пробелом либо просто начинающийся с цифр, из-за чего усечённая authority читается как `host:port`. Хост теряется вместе с ним, а `init` называет затронутые ключи в сводке фазы 1 (`envInfo.credentialWarnings`, только имена, для корневого `.env` и собственного `.env` вложенного SPA). `envInfo.host` / `envInfo.apiTarget` становятся `null`, а не несут страж в §3 CLAUDE.md. Закодируйте пароль процентами (`/` как `%2F`), чтобы хост остался виден. DSN Oracle JDBC, которые несут учётные данные перед connect descriptor, а не как URL userinfo (`jdbc:oracle:thin:scott/tiger@//dbhost:1521/ORCL`, `…@dbhost:1521:ORCL`, `…@(DESCRIPTION=…)`, `jdbc:oracle:oci:user/pw@ALIAS`), с v2.5.3 маскируются как `jdbc:oracle:thin:***/***@//dbhost:1521/ORCL` — хост, порт и имя сервиса остаются видимыми.
