...
> **Не работает с v2.1.0.** Сборка master plan удалена, поэтому `manifest-generator` пишет `sync-map.json` с пустым списком сопоставлений, а `sync-checker` завершается с `pass` ещё до того, как выполнится любой из двух шагов ниже. Здесь описан механизм в том виде, как он реализован, но он не выполняется, если только в проекте не сохранился каталог `claudeos-core/plan/` времён до v2.1.0, который `init` намеренно не трогает. `pass` от этого проверяющего означает «нечего проверять», а не «проверено».
