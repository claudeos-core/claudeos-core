...
架构的可视化参考。所有图都是 Mermaid — 在 GitHub 上自动渲染。如果你在不渲染 Mermaid 的查看器里阅读,正文说明本身故意是完整的。

只看文字版,见 [architecture.md](architecture.md)。
...
**绿色** = 代码(deterministic)。**粉色** = Claude(LLM)。两者从不在同一个工作上重叠。

---

## Pass 3 split mode

Pass 3 总是被拆分成多个 stage — 不论项目规模大小,都不会作为单次调用运行。这让每个 stage 的 prompt 即使在 `pass2-merged.json` 很大时也能保持在 LLM 的 context window 内:
...
**关键洞察:** Pass 3a 把大 input 读一次,产出一份小事实表。Stage 3b/3c/3d 只读这份小事实表,从不重读那份大 input。这避免了早期非 split 设计中常见的 "Prompt is too long" 错误。

对于 16+ 域的项目,3b 与 3c 进一步细分为每批 ≤15 域。每个 batch 都是它自己的 Claude 调用,拥有新的 context window。
...
粉色框 = Claude 被调用。菱形决策都是纯文件系统检查 — 在任何 LLM 调用之前发生。

marker 验证不仅仅是"文件存在吗?" — 每个 marker 有结构性检查(例如 Pass 4 的 marker 必须包含 `passNum === 4` 与非空 `memoryFiles` 数组)。来自上次崩溃的 malformed marker 会被拒收,该 pass 重跑。
...
`claude-md-validator` 单独运行,因为它的发现是**结构性的** — 如果 CLAUDE.md 是 malformed 的,正确做法是重跑 `init`,而不是悄悄警告。其他 validator 作为 `health` 的一部分运行,因为它们的发现是内容级(路径、manifest 条目、schema 缺口)— 那些可以在不重新生成所有内容的情况下审阅。
...
**粉色** = Claude Code 在每次会话中自动加载(无需手动加载)。其余按需加载,或被自动加载的文件引用。

`00`/`10`/`20`/`30`/`40`/`70`/`80` 的 prefix 在 `rules/` 与 `standard/` 中**都**出现 — 同一概念领域,不同角色(rule 是被加载的指令,standard 是参考文档)。数字 prefix 提供稳定排序,并允许 Pass 3 orchestrator 处理类别组(例如 60.memory 由 Pass 4 写,70.domains 按 batch 写)。真正触发 Claude Code 自动加载某条 rule 的是它 YAML frontmatter 中的 `paths:` glob,而非类别号。

`50.sync` 与 `60.memory` 是 **rules-only**(没有对应的 `standard/` 目录)。`90.optional` 是 **standard-only**(无强制力的栈相关附加)。
...
memory 文件**按需加载**,不是始终加载。这让 Claude 在常规编码中的 context 保持精简。它们仅在 rule 的 `paths:` glob 匹配 Claude 当前编辑文件时被拉入。
