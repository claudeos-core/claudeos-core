 ...
 */
/**
 * v2.5.2 — True when a `scheme://…` value looks like it carries userinfo that
 * maskUrlCredentials() cannot rewrite, because the password contains a raw
 * `/`, `?` or `#`.
 *
 * The tell: the text between `://` and the first `/`, `?` or `#` looks like
 * `user:password` (it holds a `:` and the part after that `:` is NOT all
 * digits, so it is not a `host:port`), AND an `@` appears somewhere after
 * that delimiter — i.e. the real authority ends past the point where the
 * masking regex was forced to stop.
 *
 *   postgres://u:p/w@host/db          → true   (password `p/w`)
 *   redis://:pw?x@host/0              → true   (password `pw?x`)
 *   postgres://u:p@host/db            → false  (masking already handles it)
 *   https://cdn.example.com/npm/@x/y  → false  (no `:` in the authority)
 *   http://a.com:8080/img/@2x.png     → false  (`8080` is a port, not a password)
 */
function hasUnmaskedUrlCredentials(value) {
  ...
  if (cut === -1) return false;              // no delimiter → nothing was cut short
  const authority = rest.slice(0, cut);
  const tail = rest.slice(cut);
  if (!tail.includes("@")) return false;     // authority really did end at `cut`
  const colon = authority.indexOf(":");
  if (colon === -1) return false;            // no `user:secret` shape
  const afterColon = authority.slice(colon + 1);
  if (/^\d*$/.test(afterColon)) return false; // `host:port`, not `user:password`
  return true;
}
...
function readStackEnvInfo(root) {
  ...
  return {
    ...
    // v2.5.2 — `portVars` carries the UNREDACTED value of every recognized
    // port key, so a caller that has to re-run extractPort() on a subset of
    // keys (stack-detector splits backend from frontend keys) never has to
    // reach into the redacted `vars` map. Previously it did, and no key-name
    // rule happened to intersect a port key — a future blanket rule would
    // have broken port detection silently.
    //
    // Only all-digit values are copied. A port is digits by definition, so
    // this cannot become a back-door around redaction: if a key on the port
    // whitelist ever also matches a secret-name rule, its secret value is
    // not all digits and is left out entirely.
    // v2.5.2 — keys whose value was dropped whole by the last-resort backstop
    // in maskUrlCredentials (a password holding a raw `/`, `?` or `#`). The
    // credential is not leaked, but the user loses the host for that key and
    // should percent-encode the password, so `init` names the keys in its
    // Phase 1 summary instead of silently swallowing them. Key NAMES only —
    // never any part of the value.
    // Derived from what maskUrlCredentials ACTUALLY did, so the warning can
    // never name a key whose value was kept (masked) rather than dropped.
    credentialWarnings: Object.keys(vars).filter(
      k => !isSensitiveVarName(k) && vars[k] !== "***REDACTED***" && maskUrlCredentials(vars[k]) === "***REDACTED***"
    ),
    portVars: Object.fromEntries(
      PORT_VAR_KEYS.filter(k => k in vars && /^\d+$/.test(String(vars[k]).trim()))
        .map(k => [k, vars[k]])
    ),
