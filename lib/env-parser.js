...
// v2.5.3 — the parameter rule stops its capture at `&`, `;` or whitespace,
// because those separate one connection parameter from the next. A password
// that CONTAINS one of them is therefore only half-masked and its tail rides
// out in clear (`?password=a&b` → `?password=***&b`). The tail cannot be
// masked blindly: `&sslmode=require` really is the next parameter. So the
// remainder is inspected — every `&`/`;`/space-separated chunk after the
// password value must itself look like `key=`; the first one that does not is
// password residue, and the whole value is dropped rather than published with
// a readable fragment of the secret.
const PARAM_NAME_RE = /[?&;](?:password|passwd|pwd|pass|secret|token|access[_-]?key|secret[_-]?key|api[_-]?key|sas|signature)=/gi;
const LOOKS_LIKE_PARAM_RE = /^[A-Za-z_][A-Za-z0-9_.\-]*=/;
function hasMaskedParamResidue(value) {
  PARAM_NAME_RE.lastIndex = 0;   // module-level regex with /g — reset before every scan
  let m;
  while ((m = PARAM_NAME_RE.exec(value)) !== null) {
    const chunks = value.slice(m.index + m[0].length).split(/[&;\s]/);
    for (let i = 1; i < chunks.length; i++) {
      if (chunks[i] === "") continue;
      if (!LOOKS_LIKE_PARAM_RE.test(chunks[i])) return true;
    }
  }
  return false;
}

// A host is letters, digits, `.`, `-`, `_`, a `:port`, `[]` for IPv6 and `%`
// for a zone id — nothing else, and the port is digits. Used to sanity-check
// what the userinfo rule decided was the host; see the call site.
//
// The port test is what separates the two failure shapes. `usr@tenancy:ab/cd@db/app`
// makes the rule stop at the username's `@` (its userinfo group may not cross a
// `/`), leaving `tenancy:ab` as the "host" — character-wise that passes, but
// `ab` is not a port, so the boundary was guessed wrong. A genuine
// `postgres://u:p@host/path/@x`, whose `@` merely sits in the PATH, keeps host
// `host` and is left alone.
// `,` is part of the grammar, not a typo: a MongoDB replica set and a Kafka
// broker list both put several `host:port` pairs in the authority
// (`mongodb://u:p@host1:27017,host2:27017,host3:27017/db`). Every one of them
// must check out, or the whole authority is suspect.
const HOSTISH_RE = /^[A-Za-z0-9._\-:[\]%,]+$/;
function looksLikeHost(h) {
  if (!HOSTISH_RE.test(h)) return false;
  return h.split(",").every(one => {
    if (!one) return false;
    const i = one.lastIndexOf(":");
    if (i === -1) return true;                                   // no port
    if (one.startsWith("[") && one.indexOf("]") > i) return true; // bare IPv6, `[::1]`
    return /^\d+$/.test(one.slice(i + 1));                        // `host:5432`
  });
}

function maskUrlCredentials(value) {
  if (typeof value !== "string") return value;
  if (hasMaskedParamResidue(value)) return REDACTED;
  ...
  if (/^[a-z][a-z0-9+.:-]*:\/\//i.test(value)) {
    const userinfo = value.match(USERINFO_RE);
    const userinfoMasked = !!userinfo;
    // v2.5.3 — the userinfo rule can fire on the WRONG `@`. Its userinfo group
    // may not contain `/`, so when the username holds an `@` AND the password
    // holds a `/` (`mongodb+srv://usr@tenancy:pa/ss@cluster/db`) the match ends
    // at the username's `@` and the rest of the password becomes the "host".
    // `userinfoMasked` is then true, which is exactly the gate that skips the
    // v2.5.2 backstop below — so the value sailed through half-masked with the
    // password in plain view. Checking that the captured host actually looks
    // like a host catches it: `tenancy:pa` does, `tenancy:pa/ss` cannot appear,
    // but `1b''.Y^'.` does not. Anything else means the boundary was guessed
    // wrong and the whole value goes.
    if (userinfo && !looksLikeHost(userinfo[3])) return REDACTED;
    const masked = value
  ...
  // or `0:30@daily`.
  // v2.5.3 — the rule needs a username free of `@`, `:`, `/` and whitespace.
  // A DSN that plainly carries credentials (`…@tcp(` / `…@unix(`) but does not
  // match — an `@` inside the username is the common one — used to pass
  // through with the password in clear. Drop it whole, like every other
  // boundary this file cannot resolve.
  // Test whether the rule MATCHES, not whether it changed the string. An
  // already-masked `***:***@tcp(h:3306)/db` matches and rewrites to itself, so
  // a "did anything change" test read that as "could not parse" and dropped a
  // value that was already safe — masking was not idempotent.
  const GO_DSN_RE = /^([^:@/\s]+):(.*)@(?=(?:tcp|unix)\()/;
  if (!GO_DSN_RE.test(value) && /@(?:tcp|unix)\(/.test(value)) return REDACTED;
  return value.replace(GO_DSN_RE, "***:***@").replace(PARAM_RE, "$1***");
}
