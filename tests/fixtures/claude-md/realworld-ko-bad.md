...
> 매 편집마다 자동 로드되는 공통 가드레일과, 온디맨드로 참조되는 장기 메모리 파일.

### 공통 규칙 (모든 편집 시 자동 로드)

| 규칙 파일 | 역할 |
|---|---|
| `.claude/rules/00.core/51.doc-writing-rules.md` | 문서 작성 규칙 — `paths` 프론트매터 필수 / 도메인·절대경로 하드코딩 금지 / `## 참고` 섹션 필수 / 주제 반복은 의도된 강화 |
| `.claude/rules/00.core/52.ai-work-rules.md` | AI 작업 규칙 — 분석된 사실만 사용 / 환각 방지 13개 점검 / 파일 수정 전 Read / 검증 가능한 결과 우선 |

세부 작성 절차는 `claudeos-core/standard/00.core/04.doc-writing-guide.md` 를 참고한다.

### L4 메모리 파일

| 파일 | 목적 | 동작 |
|---|---|---|
| `claudeos-core/memory/decision-log.md` | 설계 선택의 "이유"에 대한 영구 기록 | Append-only. 세션 시작 시 훑어보고, 아키텍처 변경 전 Read. 결정이 내려질 때마다 새 항목 추가 |
| `claudeos-core/memory/failure-patterns.md` | 반복되는 오류와 해결책 | 세션 시작 및 오류 발생 시 검색. 2회 이상 반복 + 원인 비자명 + 해결책 미문서화 시에만 기록 |
| `claudeos-core/memory/compaction.md` | 4단계 압축 정책 선언 | 정책 자체가 바뀔 때만 수정. 압축은 오직 `npx claudeos-core memory compact` 로 실행 |
| `claudeos-core/memory/auto-rule-update.md` | 기계 생성 rule 변경 제안 | 검토 → 수락 → rule 파일 편집 → `decision-log.md` 기록. 본문 직접 편집 금지. `npx claudeos-core memory propose-rules` 로 재생성 |

### 메모리 워크플로우

1. **세션 시작**: `failure-patterns.md` 를 스캔하여 빈번한 오류 패턴을 인지한다.
2. **최근 결정 훑어보기**: `decision-log.md` 의 최근 항목을 확인해 이미 합의된 결정을 덮어쓰지 않도록 한다.
3. **새 결정 기록**: 라이브러리 채택/거부, 컨벤션 확정, 경쟁 패턴 선택 등이 있으면 `decision-log.md` 에 append.
4. **반복 오류 기록**: 같은 오류가 2회 이상 + 원인이 비자명하면 새 pattern-id 로 `failure-patterns.md` 에 추가.
5. **주기적 압축**: 메모리 파일이 400 라인에 근접하거나 한 달 이상 정리되지 않았으면 `npx claudeos-core memory compact` 실행.
6. **rule-update 제안 검토**: `auto-rule-update.md` 에서 confidence ≥ 0.70 제안을 검토. 수락 시 해당 rule 파일 편집 후 `decision-log.md` 에 기록.
